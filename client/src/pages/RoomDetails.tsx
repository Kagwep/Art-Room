  ...
  const createScene = async (canvas: HTMLCanvasElement | null): Promise<{ scene: Scene | undefined}> => {
   ...
   // camera
	...
	document.addEventListener("webkitpointerlockchange", pointerlockchange, false);
      ...
      // Call the function
      const {meshes} = await loadModels('artroom2.glb');
    ...
    meshes[7].unfreezeWorldMatrix();
       ...
       meshes[143].unfreezeWorldMatrix();
    ...
    //   // Load the texture
      ...
      const textureOne = new Texture("https://res.cloudinary.com/dydj8hnhz/image/upload/v1713923004/DALL_E_2024-04-24_04.42.36_-_A_serene_mystical_forest_with_a_small_stream_flowing_gently_through_it._The_forest_is_dense_with_tall_ancient_trees_their_trunks_covered_in_moss._S_kshlpu.jpg", scene);
    ...
    //   // Assign the material to the mesh
     ...
     // hero.checkCollisions = true;
      ...
      // Assuming you have a fully setup Babylon.js scene and a working camera

      // Create a sound object
      var sound = new Sound("Rain", "https://res.cloudinary.com/dydj8hnhz/video/upload/v1713915852/birds-singing-calm-river-nature-ambient-sound-127411_jn2k5u.mp3", scene, null, {
        autoplay: false,
        spatialSound: true
      });


      var paintStep = meshes[143]; // Reference to the mesh that acts as the paint step

      scene.onBeforeRenderObservable.add(() => {
          var playerPosition = camera.position; // Assuming the camera represents the player's position
      
          // Get the position of the paint step mesh
          var paintStepPosition = paintStep.absolutePosition;
      
          // Calculate the horizontal (x,z) distance
         ...
         // console.log(distance)
          ...
          var tolerance_above = 3; // Adjust this value based on how precise the alignment needs to be
          var tolerance_below = 0;
      
          // Check if the distance is within the tolerance
          if ( distance >=tolerance_below && distance <= tolerance_above) {
              if (!sound.isPlaying) {
                  sound.play();
              }
          } else {
